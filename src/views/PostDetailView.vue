...
async function fetchPost() {
  ...
  try {
    ...
    const [postRes, commentsRes] = await Promise.all([
      ...
      api.get(`/posts/${route.params.postId}/comments`, { params: { page: 1 } }),
    ])

    post.value = {
      ...postRes.data,
      // Normaliza liked_by_me para isLiked — padrão interno
      isLiked: postRes.data.liked_by_me ?? false,
  ...
  <div>
    ...
    <template v-else-if="post">
      ...
      <div class="d-md-none">
        ...
        <!-- Header -->
        <div class="d-flex align-items-center gap-2 p-3" style="border-bottom: 1px solid var(--color-border);">
          <button
        ...
        <div class="d-flex align-items-center gap-3 px-3 pt-2">
          <button
            ...
            @click="handleLike"
          >
            <svg v-if="post.isLiked" viewBox="0 0 24 24" fill="#ed4956" style="width:24px;height:24px;">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg>
            <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:24px;height:24px;">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
            </svg>
          </button>
        ...
        <div class="px-3 py-2" v-if="post.caption">
          <span class="fw-semibold me-1" style="font-size:14px;">{{ post.user?.username }}</span>
          <span style="font-size:14px;">{{ post.caption }}</span>
        </div>

        <!-- Data -->
        <time class="d-block px-3 pb-2" style="font-size:11px;color:var(--color-text-muted);text-transform:uppercase;">
      ...
      <div class="d-none d-md-flex desktop-layout">
        ...
        <div class="desktop-layout__panel d-flex flex-column">
          ...
          <!-- Header -->
          <div class="d-flex align-items-center gap-2 p-3 flex-shrink-0" style="border-bottom: 1px solid var(--color-border);">
            <Avatar :src="post.user?.avatar_url" :alt="post.user?.username" size="md" />
            <RouterLink
              :to="`/profile?user=${post.user?.username}`"
          ...
          <!-- Legenda -->
          <div v-if="post.caption" class="p-3 flex-shrink-0" style="border-bottom: 1px solid var(--color-border);">
            <span class="fw-semibold me-1" style="font-size:14px;">{{ post.user?.username }}</span>
            <span style="font-size:14px;">{{ post.caption }}</span>
            <time class="d-block mt-1" style="font-size:11px;color:var(--color-text-muted);text-transform:uppercase;">
              {{ timeAgo(post.created_at) }}
            </time>
          </div>
          ...
          <div class="flex-shrink-0" style="border-top: 1px solid var(--color-border);">
